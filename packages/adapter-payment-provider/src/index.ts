...
export function createPaymentProviderAdapter({ logger, stripeApiKey }: CreatePaymentProviderAdapterInput): Payment.PaymentProviderAdapter {
    return {
        createStripePaymentIntent: async ({
            amount,
            currencyCode,
        }: Payment.Price): Promise<Payment.CreateStripePaymentIntentResponse | undefined> => {
            const client: Stripe = new Stripe(stripeApiKey, { apiVersion: '2022-11-15' });
